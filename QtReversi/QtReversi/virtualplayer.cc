...
int VirtualPlayer::value_of(const Board& board) const
{
    Color opp = opponent_of(_color);
    //第一个成分： 物质优势
    int value = board.score(_color) - board.score(opp);
    //第二个成分： 角落位置
    const int r1 = 1;
    const int c1 = 1;
    const int c = Board::kNCols;
    const int r = Board::kNRows;
    //左上角
    value += corner_value(_color, board(r1, c1),
                          board(r1 + 1, c1), board(r1, c1 + 1), board(r1 + 1, c1 + 1));
    //右上角
    value += corner_value(_color, board(r1, c),
                          board(r1 + 1, c), board(r1, c - 1), board(r1 + 1, c - 1));
    //右下角
    value += corner_value(_color, board(r, c),
                          board(r - 1, c), board(r, c - 1), board(r - 1, c - 1));
    //左下角
    value += corner_value(_color, board(r, c1),
                          board(r - 1, c1), board(r, c1 + 1), board(r - 1, c1 + 1));
    return value;
}
